...
| 6.6 | ~~`key={indice}` na lista de pares antes/depois~~ — **resolvido em 18/08/2026.** Cada par ganhou uma `chave` estável, gerada no cliente (`crypto.randomUUID`, com reserva para contexto inseguro). Não veio do banco de propósito: par recém-adicionado ainda não tem id, e o salvamento reescreve a lista inteira — o id serviria para um caso só. |
| 6.7 | ~~`/admin/clientes` não tem "apagar cliente"~~ — **resolvido em 18/08/2026.** Ação `apagarCliente` mais botão no fim de `/admin/clientes/[id]`, separado por um fio por ser a única ação irreversível da tela. A confirmação diz o tamanho do estrago antes (quantas interações e tarefas somem junto, por cascata do banco). É por aqui que se limpa o seed de demonstração sem abrir o SQL Editor — ver 1.6. |
